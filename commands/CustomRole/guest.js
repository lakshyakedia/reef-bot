...
module.exports = {
   ...
   cooldown: 5,
    ...
    run : async(client,message,args) => {
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new EmbedBuilder().setColor(message.guild.members.me.displayHexColor !== '#000000' ? message.guild.members.me.displayHexColor : client.config.embedColor).setDescription(`usage : \`${prefix}\`guest \``)]})
        }
        ...
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new EmbedBuilder().setColor(message.guild.members.me.displayHexColor !== '#000000' ? message.guild.members.me.displayHexColor : client.config.embedColor).setDescription(`there is no **guest role** set for **custom roles**`)]})
        }
